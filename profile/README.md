# Washington STEM

Washington STEM is a nonprofit organization working to make Washington State's education system more equitable — so every student, regardless of ZIP code or background, has access to the education and opportunities needed for a family-sustaining career.

We focus on three interconnected areas: **early learning and childcare access**, **K–12 STEM education equity**, and **high school to postsecondary transitions** — with a particular focus on students of color, students from low-income families, and students in rural communities.

---

## Our Public Repositories

### 🧮 [Childcare Feasibility Calculator](https://github.com/WashingtonSTEM/childcarefeasibility)
A web tool that helps childcare providers in Washington model the financial feasibility of operating a program. Covers enrollment mix, staffing, tuition, subsidies, and quality costs.
👉 [Try it live](https://childcarefeasibility.vercel.app)

---

### 🎓 [Horizons Technical Assistance Templates](https://github.com/WashingtonSTEM/Horizons_Public)
R scripts and Tableau dashboards supporting the Horizons grants — place-based partnership/initiative connecting K–12 schools, colleges, and community organizations across five Washington regions to improve student transitions into certificates, degrees, and apprenticeships.

---

### 🏫 [H2P Survey Analysis Templates](https://github.com/WashingtonSTEM/H2P_Public)
R scripts and Tableau dashboards for Washington STEM's High School to Postsecondary Collaborative (H2PC) — a partnership across 40+ schools statewide focused on equitable postsecondary preparation, dual credit access, and data-informed advising.

---

## Impact Team

The repositories here are maintained by Washington STEM's Impact team.

| Name | Role | GitHub |
|------|------|--------|
| Min Hwangbo | Impact Director | [@MinHwangbo](https://github.com/mhwangbo11) |
| Mikel Poppe | Impact Data Manager | [@MikelPoppe](https://github.com/PoppeWASTEM) |
| Rachel Tavolacci | Impact Portfolio Manager | [@RachelTavolacci](https://github.com/racheltavolacci) |

Questions about a specific repository? Each repo's README lists the best contact.

---

## Get Involved

All public templates are licensed under [CC BY-NC 4.0](https://creativecommons.org/licenses/by-nc/4.0/) — free to use and adapt for non-commercial educational purposes with attribution.

- 🌐 [washingtonstem.org](https://washingtonstem.org)
- 📬 General inquiries: [info@washingtonstem.org](mailto:info@washingtonstem.org)
- 📊 Data & impact questions: [impact@washingtonstem.org](mailto:impact@washingtonstem.org)
